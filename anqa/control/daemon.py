...
def configure_serve_logging() -> None:
    ...
    Level from ``ANQA_SERVE_LOG_LEVEL`` (default ``INFO``). Use ``DEBUG`` for
    full param-level RPC lines; ``INFO`` logs each method with timing and status.
    """
    level_name = (os.environ.get("ANQA_SERVE_LOG_LEVEL") or "INFO").strip().upper()
    level = getattr(logging, level_name, None)
    if not isinstance(level, int):
        level = logging.INFO
    root = logging.getLogger("anqa")
    root.setLevel(level)
...
def owner_protocol_probe(socket_path: Path, *, timeout: float = 2.0) -> bool | None:
    ...
    async def _probe() -> bool:
        client = ControlClient(socket_path, client_name="anqa-protocol-probe", timeout=timeout)
