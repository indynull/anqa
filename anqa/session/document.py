...
def _render_outline_document(
    ...
    *,
    ...
    if format == "org":
        ...
        for row in turns:
            preview = str(row.get("summary") or "").strip()
            pidx = _outline_prompt_index(row)
            tidx = json_as_int(row.get("turnIndex"))
            lines.extend(
                [
                    f"* Prompt {pidx}",
                    ":PROPERTIES:",
                    f":ANQA_PROMPT_INDEX: {pidx}",
                    f":ANQA_TURN_INDEX: {tidx}",
                    ":END:",
                    "",
                ]
            )
            if preview:
                lines.extend(["** User", "", *_org_fixed_lines(preview), ""])
            lines.extend(["** Operator notes", ""])
            for note in notes_by_turn.get(tidx, []):
                lines.extend(_render_note_org(note, schema))
        text = "\n".join(lines).rstrip() + "\n"
        ...
        for row in turns:
            ...
            lines.extend(
                [
                    f"## Prompt {pidx}",
                    _md_comment(**{"prompt-index": pidx, "turn-index": tidx}),
                    "",
                ]
            )
            if preview:
...
def render_editor_document(
    ...
    *,
    ...
    :param bodies: When false, turn headings and notes only (no transcript).
    :param prompt_index: Restrict to one prompt when set.
    ...
    if prompt_index is not None:
        segments = [s for s in segments if _prompt_index(s) == prompt_index]
        prompt_indexes = tuple(_prompt_index(s) for s in segments)
