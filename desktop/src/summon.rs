...
/// Session id after trim: 1..=256 bytes, no CR/LF or spaces.
pub fn sanitize_session_id(raw: &str) -> Option<String> {
    let t = raw.trim();
    if t.is_empty() || t.len() > 256 || t.contains('\n') || t.contains('\r') || t.contains(' ') {
        return None;
    }
    Some(t.to_string())
}
...
/// Canonical wire: ``verb``, ``verb token``, or ``open <sessionId>``, always one LF.
