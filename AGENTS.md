...
| ``anqa doctor`` | Host checks (config home, catalog, control owner, HUD seat) — no TUI |
...
- **Turn status** on the home list: :class:`~anqa.models.ListStatus`
  (``running`` | ``awaiting`` | ``ending`` | ``complete`` | ``cancelled`` |
  ``idle``). ``running`` is a turn in progress. ``idle`` is no list
  status (last user row or bookend); the string table paints the idle
  mark. **ending** = the last turn is still finishing after the host
  asked the session to stop. Not "window open."
